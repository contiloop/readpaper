...
Embed each discussed paper figure as an original-source image beside its explanation, with the figure number and page citation. A source link alone does not satisfy this report preference. Export the relevant PDF region without redrawing or changing its contents; preserve axes, legends, subpanels and captions. Save assets with the report, verify every Markdown image path, and visually inspect the exported figures before delivery. Use absolute local image paths for Codex display. If an original figure cannot be included, explain that specific omission. Adding missing embeds to an existing report is a presentation-only edit when its claims remain unchanged.
